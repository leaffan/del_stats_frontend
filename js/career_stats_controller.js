app.controller('careerStatsController', [
    '$scope',
    '$http',
    '$window',
    'svc',
    'config',
    'cfgLoader',
    function ($scope, $http, $window, svc, config, cfgLoader) {
        let ctrl = this;
        $scope.svc = svc;
        ...
        cfgLoader.teamsHistoric().then(function (res) {
            ...
            $scope.team_full_name_lookup = orig_teams.reduce(
                (o, key) => Object.assign(o, { [key.abbr]: key.full_name }),
                {},
            );
        });
        ...
        $http.get('data/career_stats/upd_all_players.json').then(function (res) {
            ...
            let all_countries = new Set(
                $scope.players.map((player) => player['country'].split(', ')).flat(),
            );
            $scope.most_recent_season = Math.max(
                ...$scope.players.map((player) => player['last_season']),
            );
            $http.get('./cfg/iso_countries.json').then(function (res) {
                ...
                all_countries.forEach((iso_country) => {
                    let plrs_per_country = $scope.players.filter((player) =>
                        player.country.includes(iso_country),
                    ).length;
        ...
        $http.get('data/career_stats/upd_full_career_stats_stripped.json').then(function (res) {
            ...
            let all_seasons = new Set(
                $scope.player_stats
                    .map((career) => career['seasons'])
                    .flat()
                    .map((season) => season['season']),
            );
            let all_ages = new Set(
                $scope.player_stats
                    .map((career) => career['seasons'])
                    .flat()
                    .map((season) => season['age'])
        ...
        $scope.$watchGroup(
            [
                ...
                'table_type',
                'show_only_active',
                'ctrl.display_games_with_other_teams',
        ...
        $scope.filterPlayer = function (player) {
            if (
                !['skater_career_stats', 'goalie_career_stats'].includes($scope.table_type) &&
                player['position'].includes('G')
            )
                return false;
            if (
                $scope.show_only_active &&
                config.defaultSeason == $scope.most_recent_season &&
                player['last_season'] != config.defaultSeason
            )
                return false;
            if (
                $scope.show_only_active &&
                config.defaultSeason != $scope.most_recent_season &&
                player['last_season'] != config.defaultSeason - 1
            )
                return false;
            if ($scope.position) {
                if ($scope.position == 'DE' && !player['position'].includes('D')) return false;
                if ($scope.position == 'GK' && !player['position'].includes('G')) return false;
                if (
                    $scope.position == 'FO' &&
                    (player['position'].includes('G') || player['position'].includes('D'))
                )
                    return false;
            }
            if (ctrl.from_season > player['last_season']) return false;
            if (ctrl.to_season < player['first_season']) return false;
            if ($scope.country) {
                if ($scope.country == 'non_de') {
                    if (player['country'].includes('de')) return false;
                } else {
                    if (!player['country'].includes($scope.country)) return false;
                }
            }
        ...
        $scope.filterPlayerSeasons = function (player_season, player_teams) {
            if (player_season['season'] < ctrl.from_season) return false;
            if (player_season['season'] > ctrl.to_season) return false;
            if (
                ctrl.from_age != $scope.min_age &&
                (player_season['age'] < ctrl.from_age || player_season['age'] === undefined)
            )
                return false;
            if (
                ctrl.to_age != $scope.max_age &&
                (player_season['age'] > ctrl.to_age || player_season['age'] === undefined)
            )
                return false;
            ...
            if ($scope.team) {
                if (ctrl.display_games_with_other_teams) {
                    if (!player_teams.has($scope.team)) return false;
                } else {
                    if (player_season['team'] != $scope.team) return false;
                }
            }
            return true;
        };
        ...
        $scope.preparePlayer = function (player) {
            return {
                player_id: player['c_id'] != null ? player['c_id'] : 'g' + player['g_id'],
        ...
        $scope.filterSingleSeasons = function () {
            ...
            players_of_interest.forEach((player) => {
                let player_seasons = $scope.player_stats
                    .filter(
                        (career) =>
                            career['id'] == player['g_id'] || career['id'] == player['c_id'],
                    )
                    .map((career) => career['seasons'])
                    .flat();
                let player_teams = new Set(
                    player_seasons.map((player_season) => player_season.team),
                );
                ...
                if (player_seasons.length < 1) return;
                ...
                seasons_played.forEach((season) => {
                    let player_season_stats = player_seasons.filter(
                        (single_season) => single_season.season == season,
                    );
                    let championships = player_season_stats.filter(
                        (single_season) => single_season.c == 1,
                    ).length;
                    let filtered_player_season_stats = [];
                    ['RS', 'PO'].forEach((seasonType) => {
                        let player_season_type_stats = player_season_stats.filter(
                            (single_season) => single_season.season_type == seasonType,
                        );
                        if (player_season_type_stats.length < 1) return;
                        let filtered_stat_line = prepareSingleSeasonStatline(
                            player,
                            player_season_type_stats,
                            championships,
                        );
                        stats_to_fill_up.forEach((parameter) => {
                            filtered_stat_line[parameter] = player_season_type_stats.reduce(
                                (param, season) => {
                                    return param + (season[parameter] || 0);
                                },
                                0,
                            );
                        });
                    ...
                    if ($scope.season_type == 'ALL') {
                        let all_season_stat_line = prepareSingleSeasonStatline(
                            player,
                            player_season_stats,
                            championships,
                            (combined = true),
                        );
                        sumIntoTarget(filtered_player_season_stats, all_season_stat_line, [
                            'season',
                            'age',
                            'teams_cnt',
                            'last_season',
                        ...
                        filtered_season_stats.push(
                            ...filtered_player_season_stats.filter(
                                (item) => item.season_type === $scope.season_type,
                            ),
                        );
                    }
                });
        ...
        function prepareSingleSeasonStatline(player, playerSeasonStats, championships, combined) {
            ...
            singleSeasonStatline['teams'] = Array.from(
                new Set(playerSeasonStats.map((season) => season['team'])),
            );
        ...
        $scope.filterCareerStats = function () {
            ...
            players_of_interest.forEach((player) => {
                ...
                filtered_stat_line['teams'] = new Set(
                    player_seasons.map((season) => season['team']),
                );
                ...
                if (stats_to_calculate == undefined) return;
                stats_to_aggregate.forEach((parameter) => {
                    filtered_stat_line[parameter] = player_seasons.reduce((param, season) => {
                        return param + (season[parameter] || 0);
                    }, 0);
                });
                filtered_stat_line['g_post_1998'] = player_seasons
                    .filter((season) => season.season > 1998)
                    .reduce((param, season) => {
                        return param + (season['g'] || 0);
                    }, 0);
                filtered_stat_line['ga_post_1998'] = player_seasons
                    .filter((season) => season.season > 1998)
                    .reduce((param, season) => {
                        return param + (season['ga'] || 0);
                    }, 0);
                filtered_stat_line['sa_post_1998'] = player_seasons
                    .filter((season) => season.season > 1998)
                    .reduce((param, season) => {
                        return param + (season['sa'] || 0);
                    }, 0);
                svc.calculateDerivedStats(filtered_stat_line, stats_to_calculate);
                filtered_stat_line['championships'] = player_seasons.filter(
                    (season) => season.c == 1,
                ).length;
