app.controller('homeController', function ($scope, $http, $cacheFactory) {
    ...
    $http.get('data/2023/del_player_game_stats_aggregated.json', {cache: true}).then(function (res) {
        var team_players = [...new Set(res.data[1].map(item => item.team + '/' + item.player_id))];
        $scope.random_team_player_2023 = team_players[Math.floor(Math.random() * team_players.length)];
    });
