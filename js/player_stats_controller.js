app.controller('plrStatsController', function ($scope, $http, $window, $routeParams, svc, config) {
    ...
    $http.get('./cfg/sort_criteria_player_stats.json').then((response) => {
        ...
        ctrl.sortConfig = $scope.sortConfig = {
            ...
            sortDescending: !$scope.ascendingAttrs.includes(
                $scope.sortCriteria[$scope.tableSortCriteria[ctrl.tableSelect]][0],
            ),
        };
    });
    ...
    $http.get('./cfg/teams.json').then(function (res) {
        ...
        $scope.teams.forEach((team) => {
            ...
            if (team['division'][$scope.season]) {
                for (const [season_type, division_name] of Object.entries(
                    team['division'][$scope.season],
                )) {
    ...
    $http.get('data/' + $scope.season + '/del_player_personal_data.json').then(function (res) {
        ...
        $http.get('./cfg/iso_countries.json').then(function (res) {
            ...
            countries_in_player_games = [
                ...new Set($scope.personal_data.map((item) => item.iso_country)),
            ].sort();
    ...
    $http
        .get('data/' + $scope.season + '/del_game_scores_per_game_bottom.json')
        .then(function (res) {
    ...
    $http.get('data/del_players.json').then(function (res) {
        ...
        $http
            .get(config.cloudfrontBaseUrl + $scope.season + '/del_player_game_stats.csv')
            .then($scope.preProcessData);
    ...
    $scope.preparePersonalData = function (player_game) {
        ...
        personal_data['full_name'] =
            $scope.all_players[plr_id]['first_name'] +
            ' ' +
            $scope.all_players[plr_id]['last_name'];
    ...
    $scope.filterGameScores = function (game_scores) {
        if (game_scores === undefined) return game_scores;
    ...
    $scope.filterGoalieStats = function () {
        ...
        goalie_teams = {};
        if ($scope.goalie_games === undefined) return filtered_goalie_stats;
        $scope.goalie_games.forEach((element) => {
            ...
            if ($scope.elementPassedFilters(element)) {
                ...
                $scope.categorizeStatsForStatline(
                    ctrl.goalieStatsToCategorize,
                    element,
        ...
        $scope.maxGoalieGamesPlayed = Math.max.apply(
            Math,
            filtered_goalie_stats.map(function (o) {
                return o.games_played;
            }),
        );
        $scope.maxGoalieTimeOnIce = Math.max.apply(
            Math,
            filtered_goalie_stats.map(function (o) {
                return o.toi;
            }),
        ...
        if ($scope.minGoalieGamesPlayed > 1) {
            filtered_goalie_stats = filtered_goalie_stats.filter(
                (stat_line) => stat_line.games_played >= $scope.minGoalieGamesPlayed,
            );
        }
        if ($scope.minGoalieTimeOnIce > 0) {
            filtered_goalie_stats = filtered_goalie_stats.filter(
                (stat_line) => stat_line.toi >= $scope.minGoalieTimeOnIce,
            );
        }
    ...
    $scope.categorizeStatsForStatline = function (
        statsToCategorize,
        singleGameStats,
        filteredStats,
    ) {
    ...
    $scope.processMultiTeamPlayers = function (filtered_stats, player_teams, goalies = false) {
        ...
        Object.keys(multiTeamPlayers).forEach((plr_id) => {
            ...
            multiTeamPlayerStats['full_name'] =
                plr_team_stats['first_name'] + ' ' + plr_team_stats['last_name'];
    ...
    $scope.filterStats = function () {
        ...
        player_teams = {};
        if ($scope.player_games === undefined) return filtered_player_stats;
        ...
        $scope.maxGamesPlayed = Math.max.apply(
            Math,
            filtered_player_stats.map(function (o) {
                return o.games_played;
            }),
        );
        // retrieving maximum amount of time on ice from filtered players
        $scope.maxTimeOnIce = Math.max.apply(
            Math,
            filtered_player_stats.map(function (o) {
                return o.time_on_ice;
            }),
        );
        ...
        if ($scope.minGamesPlayed > 1) {
            filtered_player_stats = filtered_player_stats.filter(
                (stat_line) => stat_line.games_played >= $scope.minGamesPlayed,
            );
        }
        // applying minimum time on ice filter
        if ($scope.minTimeOnIce > 0) {
            filtered_player_stats = filtered_player_stats.filter(
                (stat_line) => stat_line.time_on_ice >= $scope.minTimeOnIce,
            );
        }
    ...
    ctrl.greaterThanFilter = $scope.greaterThanFilter = function (prop, val) {
        return function (item) {
            ...
            if (
                ...
                ![
                    'goalie_stats',
                    'goalie_against_avg_stats',
                    'goalie_stats_ev',
                    'goalie_stats_sh',
                    'goalie_stats_pp',
                    'goalie_zone_stats_near',
                    'goalie_zone_stats_far',
                ].includes(ctrl.tableSelect)
            ) {
                return true;
            }
            if (
                prop == 'time_on_ice' &&
                !['per_game_stats', 'per_60_stats', 'time_on_ice_shift_stats'].includes(
                    ctrl.tableSelect,
                )
            ) {
                return true;
            ...
            if (
                prop == 'faceoffs_per_game' &&
                !['faceoff_stats', 'faceoff_by_side_stats', 'faceoff_by_zone_stats'].includes(
                    ctrl.tableSelect,
                )
            ) {
                return true;
            ...
            if (
                prop == 'so_games_played' &&
                !['goalie_shootout_stats', 'shootout_stats'].includes(ctrl.tableSelect)
            ) {
                return true;
            }
            if (item[prop] === undefined) return true;
    ...
    ctrl.goalieFilter = function (a) {
        if (
            [
                'player_information',
                'basic_stats',
                'bottom_game_scores_per_game',
                'top_game_scores_per_game',
            ].includes(ctrl.tableSelect)
        ) {
            return true;
        }
    ...
    ctrl.playerStatusFilter = $scope.playerStatusFilter = function (a) {
        if (!ctrl.playerStatusSelect) return true;
    ...
    ctrl.currentStreakFilter = $scope.currentStreakFilter = function (a) {
        ...
        if (!$scope.showOnlyCurrentStreak) return true;
    ...
    ctrl.teamFilter = $scope.teamFilter = function (a) {
        if ($scope.teamSelect) {
            if (['Nord', 'Süd', 'A', 'B'].includes($scope.teamSelect)) {
                if (
                    $scope.divisions[ctrl.seasonTypeSelect][$scope.teamSelect] &&
                    $scope.divisions[ctrl.seasonTypeSelect][$scope.teamSelect].includes(a.team)
                ) {
                    return true;
                } else {
                    return false;
    ...
    inputMinTimeOnIce = function () {
        if (ctrl.tableSelect.includes('goalie')) {
            ...
            $scope.minGoalieTimeOnIceInMinutesFormatted =
                svc.pad($scope.minGoalieTimeOnIceInMinutes, 2) + ':00';
        } else {
            $scope.minTimeOnIce = $scope.minTimeOnIceInMinutes * 60;
            $scope.minTimeOnIceInMinutesFormatted =
                svc.pad($scope.minTimeOnIceInMinutes, 2) + ':00';
        }
    };
