app.controller('teamProfileController', function ($scope, $http, $routeParams, $location, svc) {
    ...
    $http.get('./cfg/teams.json').then(function (res) {
        ...
        $scope.teams = $scope.teams.filter((team) => svc.isTeamValidForSeason(team, $scope.season));
        // creating lookup structures...
        // ...for team names used in urls
        $scope.team_lookup = $scope.teams.reduce(
            (o, key) => Object.assign(o, { [key.abbr]: key.url_name }),
            {},
        );
        // ...for full team names
        $scope.team_full_name_lookup = $scope.teams.reduce(
        ...
        $scope.team_playoff_lookup = $scope.teams.reduce(
            (o, key) => Object.assign(o, { [key.abbr]: key.po }),
            {},
        );
        // retrieving current team's data
        $scope.current_team_data = $scope.teams.filter(
            (team) => team.abbr == $scope.current_team,
        )[0];
        svc.setTitle(
            $scope.current_team_data.full_name +
                ': Teamprofil ' +
    ...
    $scope.setSortOrder = function (sortCriterion, oldSortCriterion, oldStatsSortDescending) {
        return svc.setSortOrder(
            sortCriterion,
            oldSortCriterion,
            oldStatsSortDescending,
            $scope.model.ascendingAttrs,
        );
    };
    ...
    $scope.getStandingsPositionThroughDate = function (cutoff_date) {
        ...
        var team_stats_pre_cutoff = $scope.team_stats.filter(
            (team_game) => moment(team_game['game_date']) <= cutoff_date,
        );
        ...
        team_stats_pre_cutoff.forEach((team_game) => {
            ...
            team_points_log[team]['pts_per_game'] =
                team_points_log[team]['pts'] / team_points_log[team]['games'];
        });
    ...
    $scope.changeTeam = function () {
        $location.path(
            '/team_profile/' + $scope.season + '/' + $scope.current_team + '/' + $scope.tableSelect,
        );
    };
