app.controller('teamStatsController', function ($scope, $http, $routeParams, $q, svc, config) {
    ...
    if ($scope.referenceSeasonSelect == '2020') $scope.referenceSeasonSelect = '2019';
    ...
    $http.get('./cfg/teams.json').then(function (res) {
        ...
        $scope.teams = res.data.filter((team) => svc.isTeamValidForSeason(team, $scope.season));
        // creating lookup structures...
        // ...for team locations
        $scope.team_location_lookup = $scope.teams.reduce(
            (o, key) => Object.assign(o, { [key.abbr]: key.location }),
            {},
        );
        // ...for playoff participation indicator
        $scope.team_playoff_lookup = $scope.teams.reduce(
            (o, key) => Object.assign(o, { [key.abbr]: key.po[$scope.season] }),
            {},
        );
    });
    ...
    $scope.filterStats = function (stats) {
        ...
        $scope.teams.forEach((team) => {
            ...
            if (!filtered_team_stats[abbr]) {
                ...
                if ($scope.season == 2020) {
                    ...
                    filtered_team_stats[abbr]['division'] =
                        team['division'][$scope.season][seasonType];
                }
        ...
        $scope.team_stats.forEach((element) => {
            ...
            if ($scope.situationSelect) {
                ...
                if (element[$scope.situationSelect]) is_selected_game_situation = true;
            } else {
                is_selected_game_situation = true;
            }
            if ($scope.seasonTypeSelect) {
                if ($scope.seasonTypeSelect === element.season_type) is_selected_season_type = true;
            } else {
            ...
            if (
                ...
                element['u23_plrs'].forEach((item) =>
                    filtered_team_stats[team]['u23_plrs'].add(item),
                );
        ...
        filtered_team_stats.forEach((element) => {
            ...
            element['u23_shifts_per_game'] = svc.calculateRate(
                element['u23_shifts'],
                element['u23_gp'],
            );
            element['u23_toi_per_game'] = svc.calculateRate(element['u23_toi'], element['u23_gp']);
            element['u23_toi_pp_sh_per_game'] = svc.calculateRate(
                element['u23_toi_pp_sh'],
                element['u23_gp'],
            );
            ...
            element['pt_pctg'] = svc.calculatePercentage(
                element['points'],
            ...
            element['shot_pctg'] = svc.calculatePercentage(
                element['goals'],
                element['shots_on_goal'],
            );
            element['opp_save_pctg'] = svc.calculatePercentage(
                element['opp_saves'],
                element['shots_on_goal'],
            );
            element['sl_og_p'] = svc.calculatePercentage(
                element['sl_og'],
                element['shots_on_goal'],
            );
            element['lf_og_p'] = svc.calculatePercentage(
                element['lf_og'],
                element['shots_on_goal'],
            );
            element['rg_og_p'] = svc.calculatePercentage(
                element['rg_og'],
                element['shots_on_goal'],
            );
            element['bl_og_p'] = svc.calculatePercentage(
                element['bl_og'],
                element['shots_on_goal'],
            );
            ...
            element['sl_sh_pctg_a'] = svc.calculatePercentage(
                element['sl_g_a'],
                element['sl_og_a'],
            );
            element['lf_sh_pctg_a'] = svc.calculatePercentage(
                element['lf_g_a'],
                element['lf_og_a'],
            );
            element['rg_sh_pctg_a'] = svc.calculatePercentage(
                element['rg_g_a'],
                element['rg_og_a'],
            );
            element['bl_sh_pctg_a'] = svc.calculatePercentage(
                element['bl_g_a'],
                element['bl_og_a'],
            );
            // calculating shooting and save percentages for shots on goal in 5-on-5 play
            element['shot_pctg_5v5'] = svc.calculatePercentage(
                element['goals_5v5'],
                element['shots_on_goal_5v5'],
            );
            element['opp_save_pctg_5v5'] = svc.calculatePercentage(
                element['shots_on_goal_5v5'] - element['goals_5v5'],
                element['shots_on_goal_5v5'],
            );
            // calculating opponent shooting, save and zone percentages for shots on goal against
            element['opp_shot_pctg'] = svc.calculatePercentage(
                element['opp_goals'],
                element['opp_shots_on_goal'],
            );
            element['save_pctg'] = svc.calculatePercentage(
                element['saves'],
                element['opp_shots_on_goal'],
            );
            element['sl_og_p_a'] = svc.calculatePercentage(
                element['sl_og_a'],
                element['opp_shots_on_goal'],
            );
            element['lf_og_p_a'] = svc.calculatePercentage(
                element['lf_og_a'],
                element['opp_shots_on_goal'],
            );
            element['rg_og_p_a'] = svc.calculatePercentage(
                element['rg_og_a'],
                element['opp_shots_on_goal'],
            );
            element['bl_og_p_a'] = svc.calculatePercentage(
                element['bl_og_a'],
                element['opp_shots_on_goal'],
            );
            // calculating opponent shooting and save percentages for shots on goal against in 5-on-5 play
            element['opp_shot_pctg_5v5'] = svc.calculatePercentage(
                element['opp_goals_5v5'],
                element['opp_shots_on_goal_5v5'],
            );
            element['save_pctg_5v5'] = svc.calculatePercentage(
                element['opp_shots_on_goal_5v5'] - element['opp_goals_5v5'],
                element['opp_shots_on_goal_5v5'],
            );
            ...
            element['opp_so_pctg'] = svc.calculatePercentage(
                element['opp_so_g'],
                element['opp_so_a'],
            );
            element['so_sv_pctg'] = svc.calculatePercentage(
                element['opp_so_a'] - element['opp_so_g'],
                element['opp_so_a'],
            );
            ...
            element['xg_pctg'] = svc.calculatePercentage(
                element['xg'],
                element['xg'] + element['opp_xg'],
            );
            element['xg_pctg_5v5'] = svc.calculatePercentage(
                element['xg_5v5'],
                element['xg_5v5'] + element['opp_xg_5v5'],
            );
            // calculating shots on goal for percentage
            element['shot_for_pctg'] = svc.calculatePercentage(
                element['shots_on_goal'],
                element['shots_on_goal'] + element['opp_shots_on_goal'],
            );
            element['opp_shot_for_pctg'] = svc.calculatePercentage(
                element['opp_shots_on_goal'],
                element['shots_on_goal'] + element['opp_shots_on_goal'],
            );
            // calculating unblocked shots (i.e. Fenwick) for percentage
            element['fenwick_events'] = element['shots_on_goal'] + element['shots_missed'];
            element['opp_fenwick_events'] =
                element['opp_shots_on_goal'] + element['opp_shots_missed'];
            element['fenwick_for_pctg'] = svc.calculatePercentage(
                element['fenwick_events'],
                element['fenwick_events'] + element['opp_fenwick_events'],
            );
            element['opp_fenwick_for_pctg'] = svc.calculatePercentage(
                element['opp_fenwick_events'],
                element['fenwick_events'] + element['opp_fenwick_events'],
            );
            // calculating shots (i.e. Corsi) for percentage
            element['corsi_for_pctg'] = svc.calculatePercentage(
                element['shots'],
                element['shots'] + element['opp_shots'],
            );
            element['opp_corsi_for_pctg'] = svc.calculatePercentage(
                element['opp_shots'],
                element['shots'] + element['opp_shots'],
            );
            ...
            element['shots_pp_2min'] = svc.calculatePer2Minutes(
                element['shots_pp'],
                element['pp_time'],
            );
            element['shots_unblocked_pp_2min'] = svc.calculatePer2Minutes(
                element['shots_unblocked_pp'],
                element['pp_time'],
            );
            element['shots_on_goal_pp_2min'] = svc.calculatePer2Minutes(
                element['shots_on_goal_pp'],
                element['pp_time'],
            );
            element['shot_pctg_pp'] = svc.calculatePercentage(
                element['pp_goals'],
                element['shots_on_goal_pp'],
            );
            // calculating penalty killing percentage
            element['pk_pctg'] =
                100 - svc.calculatePercentage(element['opp_pp_goals'], element['sh_opps']);
            // calculating special teams goal differential and combined special team percentages
            element['pp_pk_gdiff'] =
                element['pp_goals'] +
                element['sh_goals'] -
                element['opp_pp_goals'] -
                element['opp_sh_goals'];
            element['pp_pk_comb_pctg'] = element['pp_pctg'] + element['pk_pctg'];
            // calculating team faceoff percentage
            element['faceoff_pctg'] = svc.calculatePercentage(
                element['faceoffs_won'],
                element['faceoffs'],
            );
            ...
            element['avg_attendance'] = svc.calculateRate(
                element['attendance'],
                element['games_played'],
            );
            // retrieving reference season's average attendance
            element['avg_attendance_ref_season'] =
                $scope.avg_attendances[$scope.referenceSeasonSelect][element['team']];
            element['avg_attendance_delta'] =
                element['avg_attendance'] - element['avg_attendance_ref_season'];
            // calculating utilized attendance capacity
            element['util_capacity_pctg'] = svc.calculatePercentage(
                element['attendance'],
                element['capacity'],
            );
            // calculating score state percentages
            element['leading_pctg'] = svc.calculatePercentage(
                element['leading'],
                element['time_played'],
            );
            element['trailing_pctg'] = svc.calculatePercentage(
                element['trailing'],
                element['time_played'],
            );
            element['tied_pctg'] = svc.calculatePercentage(element['tied'], element['time_played']);
            // calculating 5-on-5 shot share percentages
            element['shots_5v5_pctg'] = svc.calculatePercentage(
                element['shots_5v5'],
                element['shots_5v5'] + element['opp_shots_5v5'],
            );
            element['shots_unblocked_5v5_pctg'] = svc.calculatePercentage(
                element['shots_unblocked_5v5'],
                element['shots_unblocked_5v5'] + element['opp_shots_unblocked_5v5'],
            );
            element['shots_on_goal_5v5_pctg'] = svc.calculatePercentage(
                element['shots_on_goal_5v5'],
                element['shots_on_goal_5v5'] + element['opp_shots_on_goal_5v5'],
            );
            // calculating detailed power play and penalty killing percentages
            element['pp_5v4_pctg'] = svc.calculatePercentage(
                element['ppg_5v4'],
                element['pp_5v4'],
                1,
                true,
            );
            element['pp_5v3_pctg'] = svc.calculatePercentage(
                element['ppg_5v3'],
                element['pp_5v3'],
                1,
                true,
            );
            element['pp_4v3_pctg'] = svc.calculatePercentage(
                element['ppg_4v3'],
                element['pp_4v3'],
                1,
                true,
            );
            element['pk_4v5_pctg'] = svc.calculateFrom100Percentage(
                element['opp_ppg_5v4'],
                element['opp_pp_5v4'],
            );
            element['pk_3v5_pctg'] = svc.calculateFrom100Percentage(
                element['opp_ppg_5v3'],
                element['opp_pp_5v3'],
            );
            element['pk_3v4_pctg'] = svc.calculateFrom100Percentage(
                element['opp_ppg_4v3'],
                element['opp_pp_4v3'],
            );
            // calculating powerplay/penalty killing times per game
            element['pp_time_per_game'] = svc.calculateRate(
                element['pp_time'],
                element['games_played'],
            );
            element['pk_time_per_game'] = svc.calculateRate(
                element['opp_pp_time'],
                element['games_played'],
            );
            // calculating powerplay time per powerplay goal
            element['pp_time_per_pp_goal'] = svc.calculateRate(
                element['pp_time'],
                element['pp_goals'],
            );
            element['pk_time_per_opp_pp_goal'] = svc.calculateRate(
                element['opp_pp_time'],
                element['opp_pp_goals'],
            );
            // calculating powerplays per powerplay goal
            element['full_pps_per_pp_goal'] = svc.calculateRate(
                element['pp_time_per_pp_goal'],
                120,
            );
            element['full_pks_per_opp_pp_goal'] = svc.calculateRate(
                element['pk_time_per_opp_pp_goal'],
                120,
            );
            // calculating goals per game
            element['goals_per_game'] = svc.calculateRate(
                element['goals'],
                element['games_played'],
            );
            element['opp_goals_per_game'] = svc.calculateRate(
                element['opp_goals'],
                element['games_played'],
            );
            element['ev_goals_per_game'] = svc.calculateRate(
                element['ev_goals'],
                element['games_played'],
            );
            element['opp_ev_goals_per_game'] = svc.calculateRate(
                element['opp_ev_goals'],
                element['games_played'],
            );
            element['pp_goals_per_game'] = svc.calculateRate(
                element['pp_goals'],
                element['games_played'],
            );
            element['opp_pp_goals_per_game'] = svc.calculateRate(
                element['opp_pp_goals'],
                element['games_played'],
            );
            element['streak_type'] = streaks[element['team']]['type'];
            element['streak_length'] = streaks[element['team']]['length'];
            element['streak_value'] =
                element['streak_type'] == 'l'
                    ? -1 * element['streak_length']
                    : element['streak_length'];
    ...
    $scope.changeTimespan = function () {
        ...
        if ($scope.timespanSelect === '-----------') return;
